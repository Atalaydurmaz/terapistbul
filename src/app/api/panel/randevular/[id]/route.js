...
async function createDailyRoom(roomName, expiresAt) {
  ...
  if (expiresAt) {
    body.properties.exp = Math.floor(new Date(expiresAt).getTime() / 1000) + 3600; // +1 saat
  }
...
export async function PATCH(req, { params }) {
  ...
  if (status === 'onayli') {
    try {
      ...
      // Room oluşturulamadıysa onayı iptal etme, sadece logla
    }
  }
  ...
  if (status === 'onayli' && data.email) {
    try {
      ...
      const gorusmeUrl = data.daily_room_url
        ? `${appUrl}/gorusme?room=${encodeURIComponent(data.daily_room_url)}&terapist=${encodeURIComponent(data.therapist_name || data.therapistName || '')}&name=${encodeURIComponent(data.name || '')}&id=${id}`
        : null;
      ...
      await resend.emails.send({
        ...
        subject: `✅ Randevunuz Onaylandı — ${data.therapist_name || data.therapistName}`,
        html: `<div style="font-family:sans-serif;max-width:560px;margin:0 auto;">
          <h2 style="color:#0d9488;">✅ Randevunuz Onaylandı</h2>
          <p>Sayın <strong>${data.name}</strong>,</p>
          <p><strong>${data.therapist_name || data.therapistName}</strong> ile randevunuz onaylandı.</p>
